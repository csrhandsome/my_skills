---
name: conf-papers
description: 顶级会议论文搜索推荐 - 支持 computer / hci 两套预设，搜索 CVPR/ICCV/ECCV/ICLR/AAAI/NeurIPS/ICML/CHI/UIST/CSCW/IUI/UbiComp/DIS 等顶会论文
---
You are the Conference Paper Recommender for OrbitOS.

# 目标
帮助用户搜索顶级学术会议中的论文，支持两种内置预设：
- `computer`：CVPR / ICCV / ECCV / ICLR / AAAI / NeurIPS / ICML / MICCAI / ACL / EMNLP
- `hci`：CHI / UIST / CSCW / IUI / UbiComp / DIS

按年份筛选后，生成推荐笔记到 Obsidian vault。
...
本 skill 使用独立配置文件，与 start-my-day 的 `preference.md` 完全独立。skill 目录下内置两套预设：

- `computer_conf_papers.yaml`：通用计算机/AI 方向顶会
- `hci_conf_papers.yaml`：HCI 顶会

默认使用 `computer`。如果用户明确提到 `hci`、`CHI`、`UIST`、`CSCW`、`IUI`、`UbiComp`、`DIS` 等 HCI 语境，应切换到 `hci` 预设。

推荐优先使用 `--preset computer|hci`，也可以直接传 `--config` 指向具体 yaml：

```yaml
# computer_conf_papers.yaml / hci_conf_papers.yaml
preset: computer        # 或 hci
...
default_conferences:         # 默认搜索会议
  ...
  - ...
top_n: 10                    # 返回论文数量
semantic_scholar_api_key: "..."   # 可选，但建议配置，显著减少 429
semantic_scholar_cache_path: "~/.cache/vibe_research_skills/semantic_scholar_cache.json"  # 可选
```

命令行参数（preset、年份、会议）可以覆盖配置中的默认值。如果命令行没有指定，则使用配置文件中的值。

## 步骤1：解析参数

1. **提取 preset**（可选，默认 `computer`）
   - 从用户输入中识别 `computer` / `hci`
   - 如果用户提到 `CHI`、`UIST`、`CSCW`、`IUI`、`UbiComp`、`DIS` 等 HCI 会议，也视为 `hci`

2. **提取年份**（可选，默认从配置读取）
   - 从用户输入中提取年份，如 `/conf-papers 2025`
   - 未指定时使用配置中的 `default_year`

3. **提取会议名**（可选，默认从配置读取）
   - 用户可以指定会议，如 `/conf-papers 2025 ICLR,CVPR`
   - 未指定时使用配置中的 `default_conferences`
...
```bash
cd "$SKILL_DIR"
uv run python scripts/search_conf_papers.py \
  --preset {computer|hci} \
...
> 注意：脚本默认使用 `computer` 预设；也可以显式传 `--config "$SKILL_DIR/computer_conf_papers.yaml"` 或 `--config "$SKILL_DIR/hci_conf_papers.yaml"`。`--year` 和 `--conferences` 未指定时使用配置文件中的默认值。
...
| S2 429 限流 | 统一限速 + Retry-After/指数退避；优先走 DOI 精确查询和本地缓存 |
...
**参数支持**：
- 可选：preset（`computer` / `hci`），未指定时默认 `computer`
- 可选：年份（如 `2025`），未指定时使用配置中的 `default_year`
- 可选：会议名（如 `ICLR,CVPR` 或 `CHI,UIST`，逗号分隔），未指定时使用配置中的 `default_conferences`
- 搜索关键词和排除关键词均从对应 preset 的 yaml 读取
- 示例：
  - `/conf-papers` — 使用 `computer` 预设的默认年份和会议
  - `/conf-papers 2025` — 搜索 `computer` 预设默认会议的 2025 年论文
  - `/conf-papers hci` — 使用 `hci` 预设的默认年份和会议
  - `/conf-papers 2024 ICLR` — 仅搜索 `computer` 预设下的 ICLR 2024
  - `/conf-papers hci 2024 CHI,UIST` — 搜索 `hci` 预设下的 CHI 和 UIST 2024
