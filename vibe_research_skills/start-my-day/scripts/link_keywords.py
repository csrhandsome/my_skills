...
def link_keywords_in_text(
    text: str, keyword_index: Dict[str, List[str]], existing_wikilinks: Set[str]
) -> str:
    """
    在文本中链接关键词
    ...
    sorted_keywords = sorted(
        filtered_keywords.keys(), key=lambda k: len(k), reverse=True
    )
            ...
            if note_paths:
                ...
                for match in reversed(matches):
                    ...
                    if (
                        bracket_before != -1
                        and bracket_after != -1
                        and bracket_before < start
                        and bracket_after > end
                    ):
...
def link_keywords_in_file(
    input_file: str, output_file: str, keyword_index: Dict[str, List[str]]
) -> None:
    """
    处理输入文件并输出
    ...
    for original_line, line_type, line_content, in_frontmatter in lines:
        if line_type in ["frontmatter", "code", "wikilink", "image", "link", "heading"]:
            ...
            # 处理普通文本
            processed_line = link_keywords_in_text(
                line_content, keyword_index, existing_wikilinks
            )
...
def main():
    ...
    parser.add_argument(
        "--index", type=str, required=True, help="Path to keyword index JSON file"
    )
    parser.add_argument(
        "--input", type=str, required=True, help="Input file path (markdown)"
    )
    parser.add_argument(
        "--output", type=str, required=True, help="Output file path (markdown)"
    )
